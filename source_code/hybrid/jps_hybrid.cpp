...
public:
    JPSHybrid(Grid& g, int sx, int sy, int gx, int gy, int r, int procs, int n_threads)
        : grid(g), sx(sx), sy(sy), gx(gx), gy(gy), H(g.getHeight()), W(g.getWidth()), rank(r), num_procs(procs), num_threads(n_threads) {
    ...
    vector<pair<int, int>> findPath() {
        ...
        while (active) {
            ...
            // Extract batch of work (dynamically scales to thread count)
            int batchLimit = 4096; 
            int min_f = openList.empty() ? 0 : openList.top().first;
            ...
            // Phase 1: Parallel Node Expansion using OpenMP threading inside the MPI process
            #pragma omp parallel num_threads(num_threads)
            {
                vector<vector<Msg>> thread_send(num_procs);
                struct LocalRes { int f, jp, ng, pIdx; };
                vector<LocalRes> thread_res;
                thread_res.reserve(128);
                ...
                for (int b = 0; b < (int)batch.size(); ++b) {
                    ...
                    for (int i = 0; i < 8; ++i) {
                        ...
                        if (jp != -1) {
                            ...
                            if (owner == rank) {
                                thread_res.push_back({ng + dist(jx, jy), jp, ng, idx});
                            } else {
                                thread_send[owner].push_back({jp, ng, idx});
                            }
                        }
                    }
                } // End omp for

                // Safely merge thread responses into Process structures
                #pragma omp critical
                {
                    for (int p = 0; p < num_procs; ++p) {
                        if (!thread_send[p].empty()) {
                            send_buffers[p].insert(send_buffers[p].end(), thread_send[p].begin(), thread_send[p].end());
                        }
                    }
            ...
            // Phase 2: Communication (Bulk Synchronous - only done by Master MPI thread)
        ...
        if (rank == 0 && global_found) {
            ...
            if (curr == -2) points.push_back({sx, sy}); 
...
int main(int argc, char* argv[]) {
    // Requires MPI Threading support for OpenMP inside
    int provided;
    ...
    int num_threads = max_threads; // can dynamically change it here
    ...
    if (rank == 0) {
        cout<<"========================================"<<endl;
        cout<<"Hybrid MPI + OpenMP JPS Pathfinding"<<endl;
        cout<<"Grid size: "<<W<<"x"<<H<<endl;
        cout<<"Obstacle density: "<<(density*100)<<"%"<<endl;
        cout<<"Processes: "<<procs<<" | Threads per proc: "<<num_threads<<endl;
        cout<<"========================================"<<endl;
    }
    ...
    JPSHybrid jps(grid,startX,startY,goalX,goalY,rank,procs,num_threads);
    ...
    if (rank == 0) {
        ...
        saveResults("results/performance_logs.txt","hybrid_jps",W,H,procs,num_threads,t,path.size());
        if(W<=100 && H<=100){
            exportGridWithPath("results/hybrid_jps_path.txt",grid.getData(),path,startX,startY,goalX,goalY);
            exportGridWithPathSVG("results/hybrid_jps_path.svg",grid.getData(),path,startX,startY,goalX,goalY);
        }
    }
