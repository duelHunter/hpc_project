...
void Grid::setCell(int x, int y, int value) {
    if (x >= 0 && x < height && y >= 0 && y < width) data[x][y] = value;
}

int Grid::getCell(int x, int y) const {
    if (x >= 0 && x < height && y >= 0 && y < width) return data[x][y];
    return -1;
}

void Grid::print(int maxSize) const {
    if (height > maxSize || width > maxSize) {
        std::cout << "Grid too large to print (" << width << "x" << height << ")" << std::endl;
        return;
    }
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j)
